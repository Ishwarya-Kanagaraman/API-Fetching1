let renderTable = (language, details) => {
  ...
  if (details == null) {
    ...
    if (language === "english") {
      ...
      content.innerText = `${details["eng"]} ${details["eng_exp"]}`;
    } else {
      contentNum.innerText = details["number"];
      contentSec.innerText = details["sect_tam"];
      contentChapGrp.innerText = details["chapgrp_tam"];
      contentChap.innerText = details["chap_tam"];
      content.innerText = `${details["line1"]}  ${details["line2"]}  `;
      explain.innerText = `${details["tam_exp"]}`;
    }
...
document
  ...
  .addEventListener("click", getKural, false);
